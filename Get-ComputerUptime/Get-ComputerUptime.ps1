...
Function Get-ComputerUptime {
  ...
  .EXAMPLE
    PS> Get-ComputerUptime -ComputerName SERVER1
  ...
  .INPUTS
    String. You can pass computer name via the $ComputerName parameter to Get-ComputerUptime.
  ...
  BEGIN {
    ## Use a begin block to only process this code one time if names passed from the pipeline
    $today = Get-Date ## Get the current date/time in the begin block to prevent executing Get-Date numerous times
  }

  PROCESS {
    ForEach ($computer in $ComputerName) {
      ## ComputerName is a string collection so we must be able to process each object
      Try {
        ## Wrap all code in a try/catch block to catch exceptions and to control code execution
        ...
        ## Test to ensure the computer is online. If not, set the Status to OFFLINE and throw an
        ## exception with terminates the rest of the code.
        If (-not (Test-Connection -ComputerName $computer -Count 1 -Quiet)) {
          $output.Status = 'OFFLINE'
          Throw "The computer [$($computer)] is offline."
        }

        ## Build the hashtable for the -FilterHashTable parameter. We're querying the System event log for event ID 6005
        $filterHt = @{
          'LogName' = 'System'
          'ID'      = 6005
        }
